...
combined_dfl <- c(as.data.frame(enrichedCombinedRnaseH$GO_Biological_Process_2015$Term), 
                                     as.data.frame(enrichedCombinedS9.6$GO_Biological_Process_2015$Term), 
                                     as.data.frame(enrichedCombinedShared$GO_Biological_Process_2015$Term)) 
  map(function(z) unique(z)) #remove any duplicate values


combined_dfl2 <- map(combined_dfl, function(z) !duplicated(z))


combined_dfl2 <- unique({combined_dfl[[]]<-Map(function(x) Map(unlist,x),combined_dfl);combined_dfl})

dflist2 <- lapply(combined_dfl, function(z) !duplicated(z))



un <- unlist(combined_dfl)
res <- Map(`[`, un, relist(!duplicated(combined_dfl), skeleton = combined_dfl))

typeof(combined_dfl[1])
